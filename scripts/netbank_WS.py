...
try:
    ...
    for deuda in deudas:
            ...
            if deuda['estado'] == "VIGENTE" or deuda['estado'] == "CANCELADO":
                contador_vigentes=contador_vigentes+1 
                
                payload = json.dumps({'NumeroPrestamo': deuda['numero'],  'TipoImpresion': 2,  'Carnet': ci_cliente})
                ...
                if str(response) != "<Response [500]>":
                    ...
                    f = open('{rutaPadre}descargas/{0}_{1}.pdf'.format(ci_cliente,str(deuda['relacion'])+"_Hoja_de_Riesgo_"+str(deuda['numero'])), 'wb')
                    f.write(bytes)
                    f.close()
    ...
    if contador_vigentes==0 and contador_total>0:
        ...
        payload = json.dumps({'NumeroPrestamo': no_vigente,  'TipoImpresion': 2,  'Carnet': ci_cliente})
        headers = {'Authorization': 'Bearer {0}'.format(oauth_consumer_key),'Content-Type': 'application/json'}
        response = requests.request("POST", url, headers=headers, data=payload, verify=False)
        print (response)
        print (response.text)
        if str(response) != "<Response [500]>":
            ...
            if bytes[0:4] != b'%PDF':
                raise ValueError('Missing the PDF file signature')
            f = open('{rutaPadre}descargas/{0}_{1}.pdf'.format(ci_cliente,str(deuda['relacion'])+"_Hoja_de_Riesgo_"+str(no_vigente)), 'wb')
            f.write(bytes)
            f.close()
    if contador_vigentes==0 and contador_total==0:
        now = datetime.now() 
        ...
        pdf.output('{rutaPadre}descargas/{0}_{1}.pdf'.format(ci_cliente,"Hoja_de_Riesgo"), 'F')
