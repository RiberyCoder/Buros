...
try:
    ...
    for deuda in deudas:
            ...
            if deuda['estado'] == "VIGENTE":
                ...
                if str(response) != "<Response [500]>":
                    ...
                    f = open('D:/AD/BUROS/descargas/{0}_{1}.pdf'.format(ci_cliente,"Hoja_de_Riesgo_"+str(deuda['numero'])), 'wb')
                    f.write(bytes)
                    f.close()
    ...
    if contador_vigentes==0 and contador_total>0:
        ...
        if str(response) != "<Response [500]>":
            ...
            if bytes[0:4] != b'%PDF':
                raise ValueError('Missing the PDF file signature')
            f = open('D:/AD/BUROS/descargas/{0}_{1}.pdf'.format(ci_cliente,"Hoja_de_Riesgo_"+str(no_vigente)), 'wb')
            f.write(bytes)
            f.close()
    if contador_vigentes==0 and contador_total==0:
        now = datetime.now() 
